...
var DOT_COLUMNS = [
  LOG_COLUMNS.SENSOR,
  LOG_COLUMNS.SRC_PORT,
  LOG_COLUMNS.SRC_IP,
  LOG_COLUMNS.DST_IP,
  LOG_COLUMNS.DST_PORT,
  LOG_COLUMNS.TRAIL,
  LOG_COLUMNS.PROTO,
];
var DATA_CONDENSING_COLUMNS = [
  LOG_COLUMNS.SRC_PORT,
  LOG_COLUMNS.DST_IP,
  LOG_COLUMNS.DST_PORT,
  LOG_COLUMNS.PROTO,
...
for (var column in LOG_COLUMNS)
  if (LOG_COLUMNS.hasOwnProperty(column)) LOG_COLUMNS_SIZE++;
...
$(document).ready(function () {
  ...
  DEMO =
    ...
    window.location.origin == "https://maltraildemo.github.io";
      ...
      case "report_false_positive":
        $.ajax({
          ...
          success: function (result) {
            alertify.success("Threat successfully reported to developers");
          },
        });
        break;
    }
...
function initDialogs() {
    ...
    buttons: {
      ...
      "Log In": function () {
        ...
        $.ajax({
          ...
          data:
            "username=" +
            $(this).find("#username")[0].value.trim() +
            "&hash=" +
            SHA256.hex(
              SHA256.hex($(this).find("#password")[0].value.trim()) + nonce
            ) +
            "&nonce=" +
            nonce,
          cache: false,
  ...
  $("#login_link").click(function () {
    ...
    if ($("#login_dialog").length === 0)
      $(
        '<div id="login_dialog" title="Authentication"><table><tbody><tr><td>Username:</td><td><input id="username" name="username"></td></tr><tr><td>Password:</td><td><input id="password" name="password" type="password" autocomplete="off"></td></tr></tbody></table></div>'
      )
        .appendTo("body")
        .dialog(options);
    $("#login_dialog input").val("");
  ...
  $.ajax({
    ...
    complete: function (response) {
      if (response.status === 404) {
        ...
        throw new Error(
          "Maltrail should be accessed ONLY at its server instance's address (e.g. http://127.0.0.1:8338)"
        );
      } else if (
        response.status === 200 &&
...
function toggleHeatmap() {
  if ($("#heatmap_container").is(":visible")) $("#heatmap_container").hide();
  else {
    ...
    cal.init({
      ...
      onClick: function (date, nb) {
        ...
        if (!CTRL_CLICK_PRESSED) {
          ...
          if (CTRL_DATES.length === 2) query(date, CTRL_DATES[0]);
        }
      },
    });
  } catch (err) {
...
function charTrim(str, chr) {
  while (str.substr(0, 1) === chr) str = str.substr(1);
...
function getTagHtml(tag) {
  ...
  if (tag.length > 0) {
    ...
    retval = String.prototype.concat.apply("", [
      '<span class="tag ',
      getContrastYIQ(color),
      '-label-text" style="background-color: ',
      color,
      '">',
      tag,
      "</span>",
...
function getThreatUID(threat) {
  // e.g. 192.168.0.1~>shv4.no-ip.biz
  if (threat.startsWith(FLOOD_THREAT_PREFIX))
    return (
      pad(threat.hashCode().toString(16), 6).substr(0, 6) + FLOOD_UID_SUFFIX
    );
    ...
    return (
      pad(threat.hashCode().toString(16), 6).substr(0, 6) +
      DNS_EXHAUSTION_UID_SUFFIX
    );
  else return pad(threat.hashCode().toString(16), 8);
}
...
function init(url, from, to) {
  ...
  for (var severity in SEVERITY) _SEVERITY_COUNT[SEVERITY[severity]] = 0;
  ...
  Papa.parse(DEMO ? csv : url, {
    ...
    chunk: function (results) {
      ...
      if (_ < parts.length - 1) parts[_] += " ";
      else parts[_] = " " + parts[_];
      ...
      for (var i = 0; i < total; i++) {
        ...
        if (data.length !== LOG_COLUMNS_SIZE) continue;

        var trail = data[LOG_COLUMNS.TRAIL];
        var type = data[LOG_COLUMNS.TYPE];

        if (type.match(/^[A-Z]+$/) === null) continue;

        if (!(type in TRAIL_TYPES))
          TRAIL_TYPES[type] =
            PREFERRED_TRAIL_COLORS[type] || getHashColor(type);

        if (type in COMMA_ENCODE_TRAIL_TYPES)
          trail = trail.replace(/\,/g, "&#44;");

        trail = data[LOG_COLUMNS.TRAIL] = trail
          .replace(/\\\(/g, "&#40;")
          .replace(/\\\)/g, "&#41;");
        var _ = trail.replace(/\([^)]+\)/g, "");

        if (!(_ in trailSources)) trailSources[_] = {};

        trailSources[_][data[LOG_COLUMNS.SRC_IP]] = true;

        _ += " (" + type + ")";
        if (!(_ in _TRAILS)) _TRAILS[_] = 1;
        else _TRAILS[_] += 1;
      }
      ...
      for (var i = 0; i < results.data.length; i++) {
        ...
        if (dns_exhaustion) threat_text = info + THREAT_INFIX + _;
        else if (flood) threat_text = FLOOD_THREAT_PREFIX + THREAT_INFIX + _;
        else if (dga) threat_text = src_ip + THREAT_INFIX + info;
        else if (heuristic) threat_text = src_ip + THREAT_INFIX + _ + info;
        else threat_text = src_ip + THREAT_INFIX + _;
        ...
        if (!(threat_text in _THREATS))
          threat_data = _THREATS[threat_text] = [
            ...
            time,
            data,
          ]; // count, times, minTime, maxTime, (threat)data
        else {
          ...
          if (match !== null) reduced = time.match(/ (\d+:\d+)/)[1];
          else reduced = time;

          if (reduced != last(threat_data[1])) threat_data[1].push(reduced);

          if (time < threat_data[2]) threat_data[2] = time;
          else if (time > threat_data[3]) threat_data[3] = time;
        }
        ...
        for (var j = 0; j < DOT_COLUMNS.length; j++) {
          ...
          var condensed =
            column in DATA_CONDENSING_COLUMNS && data[column].contains(",");
          if (condensed || data[column] !== _[column]) {
            if (typeof _[column] === "string") {
              ...
              if (condensed) {
                ...
                for (
                  var k = 0;
                  k < Math.min(parts.length, MAX_CONDENSED_ITEMS);
                  k++
                ) {
                  _[column][parts[k]] = true;
                }

                if (parts.length > MAX_CONDENSED_ITEMS) _[column]["..."] = true;
              } else _[column][original] = true;
            }
        ...
        if (!(src_ip in _SOURCES)) _SOURCES[src_ip] = 1;
        else _SOURCES[src_ip] += 1;

        if (!(src_ip in _SOURCE_EVENTS)) {
          _SOURCE_EVENTS[src_ip] = {};

          for (var key in TRAIL_TYPES) _SOURCE_EVENTS[src_ip][key] = 0;
        }
        ...
        if (match !== null) {
          ...
          if (!(hour in _HOURS)) {
            ...
            for (var item in TRAIL_TYPES) _HOURS[hour][item] = 0;
          }

          _HOURS[hour][type] += 1;

          if (!(threat_text in _HOURS[hour])) _HOURS[hour][threat_text] = 0;
    ...
    complete: function () {
      ...
      PAPAPARSE_COMPLETE_TIMER = setTimeout(function () {
        ...
        for (var threat_text in _THREATS) {
          ...
          if (stored_locally !== null) tagData = stored_locally.tagData;
          ...
          for (var i = 0; i < DOT_COLUMNS.length; i++) {
            ...
            if (typeof data[column] !== "string") {
              ...
              for (var entry in data[column]) {
                if (
                  column === LOG_COLUMNS.TRAIL &&
                  data[LOG_COLUMNS.TYPE] === "IP" &&
                  entry.indexOf("(") === -1
                )
                  continue;
                _.push(
                  entry.replace(
                    DATA_PARTS_DELIMITER,
                    DATA_PARTS_DELIMITER.replace(" ", "")
                  )
                );
              }

              if (
                column === LOG_COLUMNS.SRC_PORT ||
                column === LOG_COLUMNS.DST_PORT
              )
                _.sort(function (a, b) {
                  a = parseInt(a);
                  b = parseInt(b);
                ...
                column === LOG_COLUMNS.SRC_IP ||
                column === LOG_COLUMNS.DST_IP
              )
                _.sort(function (a, b) {
                  a = _ipSortingValue(a);
                  b = _ipSortingValue(b);
          ...
          for (var hour in _HOURS) {
            if (min_ === null) min_ = hour;
            else min_ = Math.min(min_, hour);

            if (max_ === null) max_ = hour;
            else max_ = Math.max(max_, hour);
          }
          ...
          for (var i = 0; i < 24; i++) sparkline_data.push(0);
          ...
          row = [
            threatUID,
            data[LOG_COLUMNS.SENSOR],
            [count, times],
            severity,
            minTime,
            maxTime,
            sparkline_data.join(","),
            data[LOG_COLUMNS.SRC_IP],
            data[LOG_COLUMNS.SRC_PORT],
            data[LOG_COLUMNS.DST_IP],
            data[LOG_COLUMNS.DST_PORT],
            data[LOG_COLUMNS.PROTO],
            data[LOG_COLUMNS.TYPE],
            data[LOG_COLUMNS.TRAIL],
            data[LOG_COLUMNS.INFO],
            data[LOG_COLUMNS.REFERENCE],
            tagData,
          ];
        ...
        if (DEMO) {
          ...
          if (_DATASET.length > 0)
            alertify.success(
              "Processed " + numberWithCommas(_TOTAL_EVENTS) + " events"
            );
          else alertify.log("No events found");

          var period = "";

          if (typeof from !== "undefined") {
            period += formatDate(from);
            if (typeof to !== "undefined") period += "_" + formatDate(to);
          }
          ...
          if (_.indexOf("seconds") != -1) _ = "today";
        ...
        if (HIDDEN_THREAT_COUNT > 0)
          alertify.log(
            HIDDEN_THREAT_COUNT + " threats are hidden in details table"
          );
...
function resetStatusButtons() {
  ...
  $("li.status-button").each(function () {
    $(this).css(
      "text-shadow",
      "-1px -1px 0 rgba(0, 0, 0, 0.50), -1px 1px 0 rgba(0, 0, 0, 0.50), 1px 1px 0 rgba(0, 0, 0, 0.50), 1px -1px 0 rgba(0, 0, 0, 0.50)"
    );
...
function tagInputKeyUp(event, forcedelete) {
  ...
  if (typeof forcedelete !== "undefined" || event.keyCode === 8) {
    // appendFilter or Delete
    if (typeof newTag === "undefined" || newTag.length === 0) {
      ...
      if (tagData.length > 0) {
        if (event.target.classList.contains("tag-input")) {
          ...
          if (i > 0) tagData = tagData.substring(0, i);
          else tagData = "";
    ...
    if (newTag.length > 0) {
      ...
      if (
        !new RegExp("(^|\\|)" + escapeRegExp(newTag) + "($|\\|)").test(tagData)
      )
...
function _sort(obj) {
  ...
  for (var key in obj) {
    if (typeof obj[key] !== "function") tuples.push([key, obj[key]]);
  }
...
function copyEllipsisToClipboard(event) {
  if (event.button === 0) {
    ...
    var common = html
      .replace(/<span class="(ipcat|hidden)">[^<]+<\/span>/g, "")
      .replace(/<[^>]+>/g, "");
    if (!text) {
        ...
        if (common) {
          ...
          for (var i = 0; i < _.length; i++) {
            if (left) _[i] += common;
              ...
              if (!common.endsWith(" ")) _[i] = common + _[i];
            }
          }
  ...
  try {
    ...
    if (typeof event !== "undefined") {
      ...
      if (event.button === 0) {
        // left mouse button
        if (
          !new RegExp("\\b" + escapeRegExp(filter) + "\\b").test(currentFilter)
        ) {
      ...
      } else if (istag === true && event.button === 1) {
        // middle mouse button
    ...
    $(".ui-tooltip").remove();
    $(".custom-menu").hide();
    $("#details_filter label input").get(0).scrollLeft = $(
      "#details_filter label input"
    ).get(0).scrollWidth;
...
function initDetails() {
  var details = $("#details").dataTable({
    ...
    columnDefs: [
      {
        orderData: [
          DATATABLES_COLUMNS.SPARKLINE,
          DATATABLES_COLUMNS.SEVERITY,
          DATATABLES_COLUMNS.SRC_IP,
          DATATABLES_COLUMNS.EVENTS,
        ],
        targets: DATATABLES_COLUMNS.SPARKLINE,
      },
      {
        orderData: [DATATABLES_COLUMNS.SENSOR, DATATABLES_COLUMNS.EVENTS],
        targets: DATATABLES_COLUMNS.SENSOR,
      },
      {
        orderData: [DATATABLES_COLUMNS.SEVERITY, DATATABLES_COLUMNS.EVENTS],
        targets: DATATABLES_COLUMNS.SEVERITY,
      },
      {
        orderData: [DATATABLES_COLUMNS.INFO, DATATABLES_COLUMNS.EVENTS],
        targets: DATATABLES_COLUMNS.INFO,
      },
      {
        orderData: [DATATABLES_COLUMNS.REFERENCE, DATATABLES_COLUMNS.EVENTS],
        targets: DATATABLES_COLUMNS.REFERENCE,
      },
      {
        orderData: [DATATABLES_COLUMNS.SRC_IP, DATATABLES_COLUMNS.EVENTS],
        targets: DATATABLES_COLUMNS.SRC_IP,
      },
      {
        orderData: [DATATABLES_COLUMNS.DST_IP, DATATABLES_COLUMNS.EVENTS],
        targets: DATATABLES_COLUMNS.DST_IP,
      },
      {
        orderData: [DATATABLES_COLUMNS.TYPE, DATATABLES_COLUMNS.EVENTS],
      ...
      //             },
      {
        render: function (data, type, row) {
          var name = "";
          if (data === SEVERITY.LOW) name = "low";
          else if (data === SEVERITY.HIGH) name = "high";
          else if (data === SEVERITY.MEDIUM) name = "medium";
          var retval =
            "<span class='severity' style='background-color: " +
            SEVERITY_COLORS[data] +
            "' value='" +
            data +
            "'>" +
            name +
      ...
      },
      {
        render: function (data, type, row) {
          if (data.indexOf(",") > -1) {
            ...
            data =
              "<span title='" +
              parts.join(DATA_PARTS_DELIMITER) +
              "' class='ellipsis'></span>";
          } else {
            if (data in PORT_NAMES) data = data + " (" + PORT_NAMES[data] + ")";
          }
          return data;
        },
      ...
      },
      {
        render: function (data, type, row) {
          ...
          if (
            data.indexOf(",") > -1 ||
            data.replace(/&#\d+;/g, "|").length > LONG_TRAIL_THRESHOLD
          ) {
            var common = "";
            var title = "";
            var left = false;
      ...
      },
      {
        render: function (data, type, row) {
          return (
            ...
            data[1].join(DATA_PARTS_DELIMITER) +
            "</span>"
          );
        },
      ...
      },
      {
        render: function (data, type, row) {
          var color =
            data in TRAIL_TYPES ? TRAIL_TYPES[data] : getHashColor(data);
          return (
            '<span class="label-type white-label-text" style="background-color: ' +
            color +
            '">' +
            data +
            "</span>"
      ...
      },
      {
        render: function (data, type, row) {
          ...
          for (var i = 0; i < items.length; i++)
            if (items[i] !== "0") value += 1;
          return (
            "<div class='sparkline hidden' value='" +
            value +
            "'>" +
            data +
            "</div>"
      ...
      },
      {
        render: function (data, type, row) {
          return (
            ...
            getContrastYIQ(data.substring(0, 6)) +
            '-label-text" style="background-color: #' +
            data.substring(0, 6) +
            '">' +
            data +
        ...
        render: function (data, type, row) {
          if (data.contains("(+")) {
            ...
            if (duplicates !== null) {
              ...
              return (
                (data.substr(0, 1) != "(" ? "<i>" + data + "</i>" : data) +
                '<span class="duplicates">' +
                duplicates[1] +
                "</span>"
              );
            }
      ...
      },
      {
        width: "1%",
        targets: [
          DATATABLES_COLUMNS.THREAT,
          DATATABLES_COLUMNS.SENSOR,
          DATATABLES_COLUMNS.EVENTS,
          DATATABLES_COLUMNS.SEVERITY,
          DATATABLES_COLUMNS.FIRST_SEEN,
          DATATABLES_COLUMNS.LAST_SEEN,
          DATATABLES_COLUMNS.SPARKLINE,
          DATATABLES_COLUMNS.SRC_IP,
          DATATABLES_COLUMNS.SRC_PORT,
          DATATABLES_COLUMNS.DST_IP,
          DATATABLES_COLUMNS.DST_PORT,
          DATATABLES_COLUMNS.PROTO,
          DATATABLES_COLUMNS.TYPE,
        ],
      },
    ],
    oLanguage: {
      ...
      sInfo:
        "Showing _START_ to _END_ of <span class='details_total'>_TOTAL_</span> threats",
      sInfoEmpty:
        "Showing 0 to 0 of <span class='details_total'>0</span> total threats",
      sInfoFiltered:
        "(<span style='color: red'>filtered</span> from _MAX_ total threats)",
          ...
          aButtons: [
            {
              ...
              fnClick: function (nButton, oConfig, oFlash) {
                ...
                var html =
                  '<table id="table_aliases" class="dataTable"><thead><tr class="ui-widget-header"><th>IP</th><th>Alias</th></tr></thead><tbody>';
                var stored = $.jStorage.get(STORAGE_KEY_EDIT_ALIASES);
                if (stored !== null) {
                ...
                $('<div id="dialog-aliases" title="Aliases"></div>')
                  .appendTo("body")
                  .html(html)
                  .dialog({
                    resizable: false,
                    ...
                    buttons: {
                      Add: function () {
                        $("#table_aliases tr:last").after(
                          '<tr class="alias"><td>&nbsp;</td><td>&nbsp;</td></tr>'
                        );
                        _initAliases();
                    ...
                    close: function (event, ui) {
                      ...
                      $(this)
                        ...
                        .each(function () {
                          ...
                          if ($.trim(ip) && $.trim(alias)) result[ip] = alias;
                        });
            ...
            },
            {
              ...
              fnClick: function (nButton, oConfig, oFlash) {
                $(
                  '<div id="dialog-confirm" title="Flush local storage?"></div>'
                )
                  .appendTo("body")
                  .html(
                    '<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>These items will be permanently deleted and<br>won\'t be recoverable. Are you sure?</p>'
                  )
                  .dialog({
                    resizable: false,
                    width: "auto",
    ...
    fnDrawCallback: function (oSettings) {
      ...
      $(".sparkline:contains(',')").sparkline("html", {
        type: "bar",
        barWidth: 2,
        barColor: SPARKLINE_COLOR,
        disableInteraction: false,
        tooltipClassname: "sparkline-tooltip",
      }); //, chartRangeMin: 0, chartRangeMax: _MAX_SPARKLINE_PER_HOUR });
    ...
    fnRowCallback: function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
      function nslookup(event, ui) {
        ...
        if (match !== null) {
          ...
          $.ajax(
            "https://stat.ripe.net/data/dns-chain/data.json?resource=" + ip,
            { dataType: "jsonp", ip: ip }
          ).done(function (json) {
            ...
            $.ajax(
              ...
              if (found !== null) {
                for (var j = 0; j < json.data.records[found].length; j++) {
                  msg +=
                    json.data.records[found][j].key +
                    ": " +
                    json.data.records[found][j].value;
                  msg += "<br>";
                }
      ...
      $.each([DATATABLES_COLUMNS.TRAIL], function (index, value) {
        ...
        if (html.indexOf("ipcat") > -1) return false;
        ...
        if (!DEMO && !isLocalAddress(ip)) {
          if (!(ip in CHECK_IP)) {
            ...
            $.ajax("/check_ip?address=" + ip, {
              ...
              var span_ip = $(
                json.ipcat.length > 0
                  ? "<span class='ipcat'></span>"
                  : "<span class='ipcat hidden'></span>"
              ).html(json.ipcat);
              ...
              if (json.worst_asns === "true")
                this.cell.append(
                  $(
                    "<span class='worst_asns' title='malicious ASN'></span>"
                  ).tooltip()
                );
            });
            ...
            interval = setInterval(
              function (ip, cell) {
                if (typeof cell === "undefined") clearInterval(interval);
                else {
                  html = cell.html();
                  if (CHECK_IP[ip] !== null) {
      ...
      $.each(
        [DATATABLES_COLUMNS.SRC_IP, DATATABLES_COLUMNS.DST_IP],
        function (index, value) {
          ...
          var options = {
            content: "please wait...",
            open: nslookup,
            position: { my: "left center", at: "right+10 top-50" },
          };
          ...
          if (!isLocalAddress(ip)) {
            if (!(ip in IP_COUNTRY)) {
              IP_COUNTRY[ip] = null;
              $.ajax(
                "https://stat.ripe.net/data/geoloc/data.json?resource=" + ip,
                { dataType: "jsonp", ip: ip, html: html, cell: cell }
              ).done(function (json) {
                var span_ip = $("<span title=''/>").html(this.html + " ");
                var country = null;

                try {
                  country = json.data.located_resources[0].locations[0].country
                    .toLowerCase()
                    .split("-")[0];
                } catch (err) {
                  try {
                    country = json.data.located_resources[0].location
                      .toLowerCase()
                      .split("-")[0];
                  } catch (err) {}
                }
                ...
                if (country !== null && country !== "ano") {
                  ...
                  img =
                    '<img src="images/blank.gif" class="flag flag-' +
                    IP_COUNTRY[this.ip] +
                    '" title="' +
                    IP_COUNTRY[this.ip].toUpperCase() +
                    '">';
                } else {
                  IP_COUNTRY[this.ip] = "unknown";
                  img =
                    '<img src="images/blank.gif" class="flag flag-unknown" title="UNKNOWN">';
                }
              ...
              interval = setInterval(
                function (ip, cell) {
                  if (typeof cell === "undefined") clearInterval(interval);
                    ...
                    html = cell.html();
                    if (typeof html === "undefined") clearInterval(interval);
                    else if (IP_COUNTRY[ip] !== null) {
            ...
            img =
              '<img src="images/lan.gif" height="11px" style="margin-bottom: 0px" title="LAN">';
  ...
  details.on("mouseenter", ".trail-text", function (event) {
    ...
    if (event.target.classList.contains("trail-text")) {
      clearTimeout(SEARCH_TIP_TIMER);
      SEARCH_TIP_TIMER = setTimeout(
        function (cell, event) {
          if (event.buttons === 0 && $(".ui-tooltip").length === 0) {
            var query = cell[0].innerHTML
              .replace(/<span class="ipcat.+span>/g, "")
              .replace(/<[^>]+>/g, "")
              .replace(/[\d.]+ \(([^)]+)\)/, "$1")
              .replace(/[()]/g, "")
              .replace(/^www\./g, "")
            ...
            $("body").append(
              $(
                '<div class="ui-tooltip searchtip"><div><img src="images/newtab.png" style="cursor: pointer" title="open in new tab"><img src="images/close.png" style="cursor: pointer; width: 16px; height: 16px" title="close"></div><iframe src="' +
                  SEARCH_TIP_URL.replace("${query}", query) +
                  '"></iframe><div>'
              )
            ...
            $(".searchtip img[title='open in new tab']").on(
              "click",
              function () {
                searchTipToTab(query);
              }
            );
            $(".searchtip img[title='close']").on("click", function () {
              $(".searchtip").remove();
            });
          }
        },
  ...
  details.on("dblclick", "td", function () {
    ...
    if ($(this).find(".info-input").length > 0)
      ...
      filter = $(this)
        .find(".trail-text")[0]
        .lastChild.textContent.replace(/\([^)]+\)/g, "");
      ...
      if (match !== null) filter = match[1].replace(/,\ /g, " ");
      else {
        var tooltip = $(".ui-tooltip");
        if (tooltip.length > 0) filter = tooltip.html().replace(/,\ /g, " ");
      }
    } else {
...
jQuery.fn.dataTableExt.afnFiltering.push(function (settings, data, dataIndex) {
  return true;
});
...
jQuery.extend(jQuery.fn.dataTableExt.oSort, {
  ...
  "port-pre": function (a) {
    ...
    if (match !== null) x = parseInt(match[0]);
...
function setChartScale(options, maxValue) {
  if (maxValue > 0) {
    ...
    while (stepValue > 0 && stepValue === Math.floor(stepValue)) {
      ...
      if (options.scaleSteps >= 5 || stepValue === 1) break;
      else stepValue = stepValue / 2;
    }
  }
}
...
function drawInfo(type) {
  ...
  if (jQuery.isEmptyObject(_HOURS)) return;

  if (
    $("#" + type.toLowerCase() + "_count")
      .parent()[0]
      .style.border.contains("white")
  ) {
    resetStatusButtons();
    return;
  }
  ...
  if (type === "Events") {
    ...
    for (var type in TRAIL_TYPES) {
      ...
      for (var hour in _HOURS) _.push([hour >>> 0, _HOURS[hour][type]]);
      ...
      for (var i = ticks[type].length - 1; i >= 0; i--) {
        if (ticks[type][i] === 0) ticks[type][i] = null;
        else break;
      }
    ...
    var ctx = $(
      '<canvas id="chart_canvas" width="' +
        CHART_WIDTH +
        '" height="' +
        CHART_HEIGHT +
        '"></canvas>'
    )
      .appendTo("#chart_area")[0]
      .getContext("2d");
    ...
    for (var type in TRAIL_TYPES) _[type] = [];
    ...
    for (var i = 0; i < _TRAILS_SORTED.length; i++) {
      ...
      if (item[1] >= threshold) {
        ...
        data.push({
          value: count,
          label: item[0],
          color: type in TRAIL_TYPES ? TRAIL_TYPES[type] : getHashColor(type),
        });
      } else other += item[1];
    }
    ...
    for (var severity in _SEVERITY_COUNT) {
      ...
      for (var key in SEVERITY)
        if (SEVERITY[key] == severity) label = key.toLowerCase();
...
function initVisual() {
  ...
  for (var severity in _SEVERITY_COUNT) {
    if (_SEVERITY_COUNT[severity] * severity > severityMax) {
      ...
      for (var key in SEVERITY)
        if (SEVERITY[key] == severity) severityMaxName = key.toLowerCase();
    }
  ...
  for (var i = 0; i < _TRAILS_SORTED.length; i++) {
    if (_TRAILS_SORTED[i][1] >= threshold) {
      ...
      options.sliceColors.push(
        type in TRAIL_TYPES ? TRAIL_TYPES[type] : getHashColor(type)
      );
    } else other += _TRAILS_SORTED[i][1];
  }
  ...
  for (var i = 0; i < _THREATS_SORTED.length; i++) {
    ...
    } else other += _THREATS_SORTED[i][1];
  }
  ...
  if (min_ !== null && max_ !== null) {
    ...
    for (var hour = min_; hour <= max_; hour++) {
      if (!(hour in _HOURS)) {
        for (var key in sparklines) sparklines[key].push(0);

        _HOURS[hour] = {};
        for (var type in TRAIL_TYPES) _HOURS[hour][type] = 0;
      } else
        for (var key in sparklines) {
          _MAX_EVENTS_PER_HOUR = Math.max(
            _MAX_EVENTS_PER_HOUR,
            _HOURS[hour][key] | 0
          );
    ...
    for (var key in sparklines) {
      for (var i = sparklines[key].length - 1; i >= 0; i--) {
        if (sparklines[key][i] === 0) sparklines[key][i] = null;
        else break;
      }
    }
  } else {
  ...
  if (other > 0) {
    ...
    for (var key in TRAIL_TYPES) _other_counts.push(other_events[key] | 0);
  ...
  var barWidth = Math.min(
    6,
    Math.max(2, Math.floor(SPARKLINE_WIDTH / (_.length + 2) - 1))
  );

  var barColors = [];
  for (var key in TRAIL_TYPES) barColors.push(TRAIL_TYPES[key]);

  $("#sources_sparkline").sparkline(_, {
  ...
  var options = {
    ...
    tooltipClassname: "",
    chartRangeMin: 0,
    chartRangeMax: _MAX_EVENTS_PER_HOUR,
    width: SPARKLINE_WIDTH,
    height: "30",
    lineWidth: 2,
  };
  ...
  for (var key in total)
    $("#" + key.toLowerCase() + "_count").html(
      sum > 0 ? numberWithCommas(total[key]) : "-"
    );
}
...
function parseDate(value) {
  ...
  if (match !== null) retval = new Date(match[1], match[2] - 1, match[3]);
...
function query(date1, date2) {
  ...
  if (date2 === undefined) {
    ...
    var url =
      location.origin + "/events?date=" + formatDate(d1) + "_" + formatDate(d2);
